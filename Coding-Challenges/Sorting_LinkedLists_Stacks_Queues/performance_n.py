...
method1_y = []
method2_y = []
print("%-6s | %-6s | %-12s | %-6s | %7s | %-6s | %7s"
      % ("Sample", n_name, k_name,
         m1_name, "leftover", m2_name, "leftover"))
for i in range(0, n+100, 100):
    n_x.append(i)
    m1 = method_test_1.timeit(number=1000)
    l1 = leftover_leaves_1(i, k)
    method1_y.append(m1)
    m2 = method_test_2.timeit(number=1000)
    l2 = leftover_leaves_2(i, k)
    method2_y.append(m2)
    print("%-6s | %-6s | %-12s | %-6.3f | %8d | %-6.3f | %7d"
          % (sample, i, k, m1, l1, m2, l2))
    sample += 1

plt.scatter(n_x, method1_y, color='red', label=str(m1_name))
plt.scatter(n_x, method2_y, color='blue', label=str(m2_name))
plt.grid()
